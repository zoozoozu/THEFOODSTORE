...
public class CartController {
	...
	/*
	 * @RequestMapping({"order/cartAdd", "/cartAdd" }) public String addCart(Model
	 * model, HttpSession session, @RequestParam(value = "userId") String userId,
	 * 
	 * @RequestParam(value = "productNo") int productNo, @RequestParam(value =
	 * "amount") int amount) { Cart cart = new Cart(); cart.setUserId(userId);
	 * cart.setProductId(productNo); cart.setAmount(amount);
	 * cartService.addCart(cart);
	 * 
	 * // 장바구니에 기존 상품이 있는지 검사 cartService.addCart(cart);
	 * 
	...
	public String cartList(Model model, HttpSession session, @RequestParam(value = "userId") String userId,
